...
def erf_n(n):
    ...
    return max(err)
#массив максимальной погрешности по равномерным узлам
n_i, err_i = [], []
for i in range(5, 45):
    if i % 10 == 0:
        continue
        ...
        n_i += [i]
        err_i += [round(erf_n(i), 15)]
#массив максимальной погрешности по чебышевским узлам
n_ch, err_ch = [], []
for i in range(5, 45):
    if i % 10 == 0:
        continue
        ...
        n_ch += [i]
        err_ch += [round(erf_ch(i), 15)]
#выводим таблицу макс погрешности по равномерным и чебышевским
print("n       err_i           err_ch")
for i in range(len(n_i)):
    print(n_i[i]," ", err_i[i],"  ", err_ch[i])